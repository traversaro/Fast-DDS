...
class Parser:
    ...
    def __init__(self, argv):
        ...
        try:
            ...
            if argv and argv[0] not in [command.value for command in Command]:
            ...
            if command_int == command_to_int[Command.SHUTDOWN]:
                ...
                print(client_cli.run_request_nb(domain, [tool_path] + args_for_cpp))
                ...
                self.__start_daemon()
                self.__add_remote_servers_to_args(args_for_cpp)
                print(client_cli.run_request_b(domain, [tool_path] + args_for_cpp, True))
            elif command_int == command_to_int[Command.SET]:
                self.__start_daemon()
                print(client_cli.run_request_b(domain, [tool_path] + args_for_cpp, True))
            elif command_int == command_to_int[Command.LIST]:
                self.__start_daemon()
                print(client_cli.run_request_b(domain, [tool_path] + args_for_cpp, False))
                pass
    ...
    def __start_daemon(self):
        ...
        if spawn_daemon(timeout=10.0, debug=False):
    ...
    def __get_domain_id_from_env(self) -> int:
            ...
            if env_value is not None:
                try:
                    ...
                    print(f"Error: Found invalid Domain ID in environment variable: {env_value}.")
            return id
